...
<template>
	...
	<template v-if="isIOSMsg">
		<div class="modal" style="display: block" tabindex="30" aria-hidden="true">
	...
	<div
		...
		style="min-height: 100vh; height: 100vh; padding-bottom: 9rem">
		<div class="nav-bar">
			<div
				class="d-flex justify-content-between container pt-2 pb-3 flex-wrap no-540">
				<div class="d-flex gap-3 align-items-center">
					<router-link :to="{ name: 'Home' }" class="navbar-brand text-light"
						>Eagle 猜歌小遊戲</router-link
					>
					<span
						@click="copyURL()"
						...
						><div class="text-light">
							房間 #{{ $route.params.room_id }}
							<i class="fa-solid fa-arrow-up-right-from-square"></i></div
					></span>
				</div>
				<div class="d-flex align-items-center sm-nav-w-100">
					<button
						@click="playerListVisibility = true"
						class="btn btn-outline-light me-auto player-list-switch text-nowrap">
						<div class="d-flex align-items-center gap-1">
							<i class="fa-solid fa-user-group"></i>列表
						</div>
					</button>
					<div class="d-flex gap-2 align-items-center ms-1">
						<input class="name-input" v-model="playerName" />
						<button
							class="btn btn-outline-light text-nowrap circle-btn"
				...
				<div
					...
					:style="{
						width: `${
							room.playing
								? (Math.round(
										Now.diff(this.$moment.unix(room.start_datetime)) / 100
								  ) /
										10 /
										room.timePerRound) *
								  100
					...
					{{
						room.playing
							? Math.round(
									Now.diff(this.$moment.unix(room.start_datetime)) / 1000
							  )
							: room.isStart
							? "請稍等"
							: "等待回合開始"
			...
			<template v-if="room != null">
				<div class="row h-100">
					<div class="col-md-5 col-lg-3 player-list-box active h-100">
						<div class="player-list h-100">
							...
							<div class="h-100" style="position: relative">
								<ul
									class="list-group player-list-group"
									style="overflow-y: scroll; overflow-x: hidden">
									<li
										class="list-group-item player"
										v-for="user in this.room.users"
										:key="user"
										:class="{
											wrong:
										...
										<div class="d-flex">
											<span v-if="user.connected" style="color: green"
					...
					<transition
						...
						<div
							class="col-md-4 player-list-box player-list-sm w-100"
							v-if="playerListVisibility">
							<div class="player-list">
								<div
									@click="playerListVisibility = false"
									style="cursor: pointer"
					...
					<div class="w-100 col d-flex flex-column">
						<div class="pt-2 pb-2">
							<div
								class="np-card p-4 d-flex align-items-center"
								v-if="room != null">
								<div>
									<img
										:src="room.raw_data.artwork"
										width="90"
										height="90"
										class="me-3" />
								</div>
								<div class="d-flex gap-3 w-100 align-items-center flex-wrap">
									<div class="d-flex flex-column me-auto">
										{{ room.raw_data.rawName
										}}<small>{{ room.raw_data.name }}</small>
									</div>
									<div class="me-3">
										<span class="text-nowrap">
											回合 :
											<template v-if="room.isStart"
												>{{ room.current_round }}/{{
													room.total_round
												}}</template
											>
										...
										<div class="d-flex gap-1">
											...
											<button
												class="btn btn-primary w-100 mt-3"
												v-show="getUser() != null && getUser().leader"
												@click="
													SyncSetting();
													showRoomSetting = !showRoomSetting;
						...
						<div class="d-flex flex-column gap-3 mb-3">
							<a
								@click="select(song_selection.id)"
								class="btn"
								:class="{
									'btn-danger':
										getUser().current_select == song_selection.id &&
										endRound &&
										song_selection.id != room.current_song.id,
									'btn-success':
										endRound && song_selection.id == room.current_song.id,
									'btn-primary':
										getUser().current_select == song_selection.id && !endRound,
									'btn-light':
										getUser().current_select != song_selection.id &&
										!(endRound && song_selection.id == room.current_song.id),
								}"
								v-for="song_selection in room.current_song_selections"
								:key="song_selection">
								<div
									...
									{{ getSelectionCount(song_selection.id) }}
								</div>
								<div class="ms-5 me-5">
									{{ song_selection.attributes.name }}
								</div>
							</a>
						</div>
						<div
							class="mt-auto chat-box p-2 text-white d-flex flex-column mb-5"
					...
					<div class="song-list-box active h-100" style="width: 400px">
						<div class="song-list h-100">
							...
							<ul
								class="list-group song-list-group"
								style="overflow-y: scroll; overflow-x: hidden">
								<li
									class="pt-1 pb-1 song-card ps-3 pe-3"
									v-for="song in this.room.historySongs"
									:key="song">
										...
										<div class="d-flex flex-column">
											...
											<div
												...
												<small style="font-size: xx-small"
													...
													{{ song.attributes.albumName }}</small
												>
											</div>
										</div>
									</div>
	...
	<div class="bottom-bar p-2">
		<div class="container no-540 d-flex gap-3 align-items-center">
			<va-button
				color="#2550C0"
				class="me-auto"
				@click="showVolumeSetting = !showVolumeSetting"
			...
			></va-button>
			<va-button gradient color="warning"
	...
	<template>
		<va-modal v-model="showVolumeSetting" :mobile-fullscreen="false">
			<template #content>
				...
				<va-slider
					...
					:max="1.0"
					:step="0.01"
					@change="audio.volume = this.volume" />
			</template>
		</va-modal>
	</template>
	<template v-if="room != null">
		<va-modal v-model="room.isEnd" :mobile-fullscreen="false">
			<template #content>
				<h3>遊戲結束</h3>
				<ul class="list-group">
					<li
						...
						<div class="d-flex">
							...
							<div class="me-auto ms-1 d-flex align-items-center">
								<span>{{ user.name }}</span>
								<span v-show="user.leader" class="badge bg-secondary ms-1"
									>房主</span
								>
							</div>
	...
	<template v-if="room != null">
		<va-modal v-model="showRoomSetting">
			<template #content>
						...
						<div>
							<template v-if="loadPlayList.tempPlaylistInfo == null">
								<div class="pb-2">
									<select
										...
										@change="fastSelect(loadPlayList.fastSelect)"
										v-model="loadPlayList.fastSelect">
										<option value="None" selected>快速導入</option>
										<option
											:value="item.value"
											v-for="item in loadPlayList.DefaultPlaylist"
											:key="item.value">
											{{ item.name }}
										</option>
										...
										<option
											:value="item.value"
											v-for="item in loadPlayList.historyPlaylist"
											:key="item.value">
											{{ item.name }}
										</option>
									</select>
								...
								<div class="pb-2">
									...
									<input
										type="text"
										v-model="loadPlayList.id"
										class="w-100 form-control" />
								</div>
								<va-button
									:loading="loadPlayList.loadingPlaylist"
									class="w-100"
									color="#2550C0"
							...
							<template v-else>
								<va-card-content>
									<div class="d-flex">
										<img
											:src="loadPlayList.tempPlaylistInfo.artwork"
											width="90"
											height="90"
											class="me-3"
								...
								<div class="d-flex gap-2">
									<va-button
										class="w-100"
										outline
										@click="loadPlayList.tempPlaylistInfo = null"
										>取消</va-button
									>
									<va-button
										class="w-100"
										color="#2550C0"
										@click="
											roomSettings.raw_data = loadPlayList.tempPlaylistInfo;
											showListChangeModal = false;
											loadPlayList.tempPlaylistInfo = null;
										"
										>選取</va-button
									>
								</div>
				...
				<div class="p-2 d-flex flex-column gap-3">
					<va-card square outlined class="my-3">
						<va-card-content>
							<div class="d-flex">
								<img
									:src="roomSettings.raw_data.artwork"
									width="90"
									height="90"
									class="me-3"
									style="border-radius: 15px" />
								...
								<div class="ms-auto mt-auto">
									<va-button color="#2550C0" @click="showListChangeModal = true"
										>更換</va-button
									>
								</div>
							</div>
					...
					<div class="d-flex flex-wrap gap-2 mt-3">
						<va-button class="w-100" color="primary" @click="saveRoomSettings()"
							>保存</va-button
						>
						<va-button class="w-100" color="danger" gradient @click="reset()"
							>重置遊戲</va-button
						>
						<va-button class="w-100" flat @click="showRoomSetting = false"
							>關閉視窗</va-button
						>
					</div>
				</div>
...
<script>
	...
	export default {
		...
		data() {
			return {
				loadPlayList: {
					...
					historyPlaylist:
						localStorage.getItem("historyPlaylist") == undefined
							? []
							: JSON.parse(localStorage.getItem("historyPlaylist")),
					DefaultPlaylist: [
						{
				...
				(this.interval = setInterval(() => {
					if (this.room != null)
						if (this.room.playing)
							if (
								Math.round(
									this.Now.diff(this.$moment.unix(this.room.start_datetime)) /
										100
								) /
									10 <=
								10
		...
		methods: {
			fastSelect(key) {
				if (key != "None") this.loadPlayList.id = key;
			},
			get_playlist_info() {
				...
				fetch(
					"//" +
						...
						`/api/playlist/${this.loadPlayList.id}`,
					{
						headers: {
					...
					.then((data) => {
						...
						var found = this.loadPlayList.DefaultPlaylist.find(
							(element) => element.value == this.loadPlayList.id
						);
						if (found) return;
						var savedData =
							localStorage.getItem("historyPlaylist") == undefined
			...
			SyncSetting() {
				...
				this.roomSettings.song_selections_count =
					this.room.song_selections_count;
			...
			saveRoomSettings() {
				fetch(
					"//" +
						window.location.hostname +
						`/api/room/${this.$route.params.room_id}/changeSetting`,
					{
			...
			getSelectionCount(id) {
				...
				this.room.users.forEach((element) => {
					if (element.current_select == id) count++;
				});
				return count;
			},
			copyURL() {
			...
			reset() {
				fetch(
					"//" +
						window.location.hostname +
						`/api/room/${this.$route.params.room_id}/reset`,
					{
						headers: {
							"content-type": "application/json",
							Authorization: "Bearer " + this.getToken(),
			...
			select(song_id) {
				...
				fetch(
					"//" +
						window.location.hostname +
						`/api/room/${this.$route.params.room_id}/select`,
					{
			...
			getUser() {
				if (this.room != null) {
					var user = this.room.users.find(
						(element) =>
							element.hash ==
							crypto_.createHash("sha256").update(this.getToken()).digest("hex")
					);
					return user;
				}
				return null;
			...
			async init() {
				...
				await fetch(
					"//" +
						window.location.hostname +
						`/api/room/${this.$route.params.room_id}`,
					{
						headers: {
							"content-type": "application/json",
							Authorization: "Bearer " + this.getToken(),
					...
					.then((data) => {
						if (!("error" in data)) {
							this.room = data;
							// change title to room name
							document.title = `Eagle 猜歌遊戲 - ${this.room.raw_data.rawName}`;
							this.connect();
							if (!data.isEnd) this.audioLoad();
						} else {
							this.$vaToast.init({ message: "房間已解散", color: "danger" });
			...
			connect() {
				...
				ws = new WebSocket(
					(window.location.hostname == "api."
						...
						: "wss://") +
						window.location.hostname +
						`/api/room/ws/${this.$route.params.room_id}/${this.getToken()}`
				);
			...
			changePlayerName() {
				...
				fetch(
					"//" +
						window.location.hostname +
						`/api/room/${this.$route.params.room_id}/changeName`,
					{
			...
			startGame() {
				fetch(
					"//" +
						window.location.hostname +
						`/api/room/${this.$route.params.room_id}/start`,
					{
						headers: {
							"content-type": "application/json",
							Authorization: "Bearer " + this.getToken(),
